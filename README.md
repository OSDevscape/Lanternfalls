...
<a href="https://top.gg/discord/servers/877985802158972928">
  <img src="https://top.gg/api/v1/widgets/large/877985802158972928">
</a>
</p>
...
ReadQuest brings those ideas together and adds a purposeful game layer.

Your library becomes a world. Reading sessions become meaningful progress. Consistency helps build your character. The RPG systems are designed to make reading feel more rewarding—not to turn it into another noisy productivity chore.
...
| **Book Bosses** | Face deterministic encounters tied to the books in your library. |
| **Realm** | Explore ReadQuest through Character, classes, inventory systems, achievements, and the Bookwyrm Bazaar. |
...
| **Import and backup** | Import Goodreads CSV exports, import/export ReadQuest JSON, and back up or restore with Google Drive. |
...
- Cache book covers and metadata where available.
- Organize books using these status states:
  - To Read
  - Reading
  - Paused
  - Finished
...
- Restore and replace the ReadQuest book list from a Goodreads CSV after confirmation.
- Map Goodreads shelves to ReadQuest reading statuses:
  - `read` → Finished
  - `currently-reading` → Reading
  - `to-read` → To Read
- Preserve Goodreads shelves as ReadQuest tags.
- Import supported Goodreads fields including title, author, ISBN, personal rating, average rating, publisher, binding, page count, publication years, date read, date added, review text, and private notes where present.
- Match books by Goodreads Book ID when available, then ISBN, then normalized title and author.

> Goodreads import is designed for library migration and update. It does not replace a full ReadQuest backup because Goodreads does not contain ReadQuest-specific RPG progress, settings, reading-session history, or all custom library metadata.
...
- Collect loot, artifacts, trophies, achievements, and progression rewards.
- Explore class rules and class-specific identity within the wider adventure.

### Realm systems

- Visit **Character** to manage class, level, stats, and progression.
- Visit **The Bookwyrm Bazaar** for marketplace and reading-enchantment-style systems.
...
- See future-facing world destinations surfaced in the app experience:
  - Equipment
  - Collection
  - World Map
  - Dungeon Party
...
## Import and backup guide

ReadQuest supports three different data-transfer options. Each has a different purpose.

| Option | Best for | Includes | Important limitation |
| --- | --- | --- | --- |
| **Goodreads CSV Import** | Moving a Goodreads library into ReadQuest | Goodreads book, shelf, rating, publisher, pages, dates, review, and note data when present | Does not contain ReadQuest RPG data, settings, or complete app history |
...
| **Google Drive Backup** | Backing up and restoring ReadQuest across devices | Books, supported metadata, and ReadQuest settings | Manual backup and restore flow; Google Drive access is required |

### Goodreads CSV workflow

1. Export your library from Goodreads as a CSV file.
2. Open the ReadQuest Library menu.
3. Choose one of the Goodreads actions:
   - **Import Goodreads CSV** to add only books that do not already match your library.
   - **Restore and Merge from Goodreads** to add missing books and enrich matched books with missing Goodreads data.
   - **Restore and Replace from Goodreads** to replace the current ReadQuest book list with the selected Goodreads CSV.
4. Select the exported Goodreads `.csv` file.
5. Review the confirmation message before saving.

Before using **Restore and Replace from Goodreads**, create a ReadQuest JSON export or a Google Drive backup so you can return to your prior library if needed.

### Google Drive workflow

1. Open the ReadQuest Library menu.
2. Connect Google Drive.
3. Choose **Back up to Google Drive** to create or update the private ReadQuest backup.
4. On another device, choose one of the restore options:
   - **Restore and Merge from Google Drive** to combine libraries.
   - **Restore and Replace from Google Drive** to use the backed-up ReadQuest library and settings.

Google Drive backup is the recommended option before major library changes because it preserves more ReadQuest-specific data than Goodreads CSV import. The current Drive snapshot contains the book list, stored metadata, and app settings. [51]
...
| Local data | Browser local storage plus Capacitor Filesystem on Android |
...
ReadQuest is designed so your core reading library and progress remain available on-device.

- Your books are stored locally.
- Reading sessions and RPG progression are designed around local application state.
...
- Metadata lookups and cover fetching require network access when needed.

The storage layer writes to local browser storage and, on Android, maintains a native `books.json` copy through Capacitor Filesystem when that plugin is available. [64]

## Data safety

Before testing an import, large library change, or replace operation:

1. Export a local ReadQuest JSON backup.
2. Optionally run a Google Drive backup.
3. Test unfamiliar CSV files against a small sample library first.
4. Use Goodreads **Merge** before Goodreads **Replace** whenever possible.
5. Avoid clearing Android app storage unless you have verified a current backup.

## Roadmap

ReadQuest is actively developed. Current and planned areas include:

- More polished Goodreads import preview and row-level conflict resolution.
- Expanded metadata handling and cover recovery.
- Better series, collection, and library organization tools.
- More class-specific mechanics and rewards.
- Additional quests, boss behavior, achievements, trophies, and loot.
- More Realm destinations and world-building systems.
- Improved Android widget behavior and reading-session controls.
- Continued accessibility, mobile layout, and offline reliability improvements.
...
Found something broken? Open a bug report with a clear description of what happened, what you expected instead, and the steps needed to reproduce it.

When relevant, include your ReadQuest version, Android version, device model, screenshots, recordings, or logs. The more reproducible the report, the easier it is to fix.

### Suggest a feature

Have an idea that would make ReadQuest more useful, more fun, or more immersive? Open a feature request and describe:

- What you would like to see
- Why it would improve the experience
- How you imagine it working
- Any examples, screenshots, or references that communicate the idea

Ideas for reading tools, library organization, RPG progression, combat, loot, character systems, world-building, accessibility, and visual polish are all welcome.

### A few guidelines

- Search open issues before creating a new one.
- Keep one issue focused on one bug, improvement, or feature idea.
- Do not share private reading data, exported backups, account details, or access tokens.
- For import or backup issues, mention whether you used Goodreads CSV, ReadQuest JSON, or Google Drive—and whether you chose Import, Merge, or Replace.
