(function () {
  ...
  function install() {
    ...
    style.textContent =
      ...
      '.status-pages:empty{display:none}' +
    ...
    controls.onclick = function (event) {
      ...
      var cards = Array.prototype.slice.call(
        document.querySelectorAll('.book-card[data-status="' + wanted + '"]')
      );
      if (!cards.length) {
        list.innerHTML = '<p class="status-page-empty">No books in ' + label.toLowerCase() + '.</p>';
      }
      ...
      cards.forEach(function (card) {
        ...
        title.textContent = (card.querySelector('.book-title') || {}).textContent || 'Untitled';
        var author = document.createElement('span');
        author.textContent = (card.querySelector('.book-author') || {}).textContent || '';
