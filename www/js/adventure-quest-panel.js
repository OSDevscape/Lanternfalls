(function () {
  ...
  function render() {
    ...
    panel.innerHTML = '<div class="adventure-streak-row"><div><span class="adventure-label">Reading Streak</span><h2>' + streak.current + ' day' + (streak.current === 1 ? '' : 's') + '</h2><p class="adventure-muted">Best: ' + streak.longest + ' day' + (streak.longest === 1 ? '' : 's') + ' · 10 min/day</p></div></div><div class="adventure-quest-divider"></div><span class="adventure-label">Weekly Quest</span><h2>' + quest.title + '</h2><p class="adventure-muted">' + quest.detail + '</p><div class="adventure-quest-bar"><i style="width:' + Math.min(100, Math.round((Number(state.progress) || 0) / quest.target * 100)) + '%"></i></div><p class="adventure-quest-count">' + state.progress + ' / ' + quest.target + '</p><button type="button" data-adventure-claim-quest ' + (claimed || !complete ? 'disabled' : '') + '>' + (claimed ? 'Quest Claimed' : complete ? 'Claim Quest Reward' : 'Quest in Progress') + '</button>';
  ...
  return labels.indexOf('Reading Time') !== -1 &&
    labels.indexOf('Completed') !== -1;
})[0];
