(function () {
  ...
  function render() {
    ...
    var bossHtml = bosses.books.length ? '<div class="adventure-boss-rewards"><span class="adventure-label">Boss Defeat Ready</span>' + bosses.books.map(function (item) { return '<p><b>' + item.reward.bookTitle + '</b><span>+' + item.reward.xp + ' XP · +' + item.reward.gold + ' gold · Trophy · Loot drop</span></p>'; }).join('') + '</div>' : '';

    var gold = document.createElement('div');
    gold.className = 'adventure-gold-total';
    gold.innerHTML = '<strong>◉ ' + value.gold + '</strong>';
    var progress = document.createElement('div');
    progress.className = 'adventure-progression-card';
    progress.innerHTML = '<div class="adventure-xp-bar"><i style="width:' + percent + '%"></i></div><p class="adventure-xp-text">' + intoLevel + ' / ' + needed + ' XP to Level ' + (level + 1) + '</p>' + bossHtml + '<div class="adventure-reward-box"><p>' + detail + '</p><button id="claimRewards" type="button" ' + (pendingCount ? '' : 'disabled') + '>' + (pendingCount ? 'Claim ' + totalXp + ' XP · ' + totalGold + ' Gold' : 'No Rewards Ready') + '</button></div><details class="adventure-stats" ' + (points ? 'open' : '') + '><summary>Stats <em>' + points + ' point' + (points === 1 ? '' : 's') + ' available</em></summary><div class="adventure-stat-grid">' + STATS.map(function (stat) { return '<div><span>' + stat.toUpperCase() + '</span><b>' + value.stats[stat] + '</b><button type="button" data-adventure-stat="' + stat + '" ' + (points ? '' : 'disabled') + '>+</button></div>'; }).join('') + '</div></details>';
