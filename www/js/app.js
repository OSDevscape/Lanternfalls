(() => {
  ...
  const formats = {
    paperback: 'Paperback',
    'hardback-special': 'Hardback (Special)',
    ebook: 'Ebook',
    kindle: 'Kindle',
    audiobook: 'Audiobook'
  };
  ...
  const cover = isbn => {
    ...
    return value
      ? `https://covers.openlibrary.org/b/isbn/${encodeURIComponent(value)}-M.jpg?default=false`
      : '';
  };
  ...
  function setStatus(value) {
    ...
    el.statuses.querySelectorAll('.stamp').forEach(button => {
      button.classList.toggle(
        'selected',
        normalizeStatus(button.dataset.status) === selectedStatus
      );
    });
  }
  ...
  function setRating(value) {
    ...
    el.ratings.querySelectorAll('.star').forEach(button => {
      button.classList.toggle(
        'filled',
        +button.dataset.star <= value
      );
    });
  }
  ...
  function card(book) {
    ...
    const selectedCover = secureCoverUrl(book.coverUrl);
    const url = selectedCover || cover(book.isbn);
    ...
    if (url) {
      ...
      if (!selectedCover && window.BookCoverCache) {
        window.BookCoverCache.attach(image, book.isbn);
      }
    }
    ...
    text.innerHTML =
      '<p class="book-title"></p><p class="book-author"></p>';
  ...
  function render() {
    ...
    if (!filtered.length) {
      el.list.textContent =
        `No books match “${el.search.value.trim()}”`;

      return;
    }

    filtered.forEach(book => {
      el.list.append(card(book));
    });
  }
  ...
  function open(book) {
    ...
    setFieldValue('fieldCoverUrl', book && book.coverUrl);
  ...
  async function save() {
    ...
    const book = {
      ...
      coverUrl: secureCoverUrl(fieldValue('fieldCoverUrl')),
      dateAdded: prior?.dateAdded || new Date().toISOString()
    };

    books = editingId
      ? books.map(item => item.id === editingId ? book : item)
      : [...books, book];
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    books = (await S.loadBooks()).map(book => ({
      ...book,
