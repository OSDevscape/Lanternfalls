(function () {
  ...
  function trophyDate(event, book, sessions) {
    ...
    return (
      ...
      latest.createdAt ||
      latest.endedAt ||
      (book && (book.dateFinished || book.dateAdded))
    );
  }
  ...
  function bossName(book, event) {
    if (
      ...
      return window.BookShelfBosses.get(book).name;
    }

    return event.bossName || 'Book Boss';
  }
  ...
  async function render() {
    ...
    if (
      ...
      !page.classList.contains('collection-page') ||
      page.dataset.collectionTrophyDetailsRendering
    ) {
      return;
    }

    var list = page.querySelector('#collectionTrophyList');

    if (!list) {
      return;
    }

    page.dataset.collectionTrophyDetailsRendering = 'true';

    var events = read(LOOT_KEY, '{"events":[]}').events || [];
    var books = [];

    try {
      books = await window.BookStorage.loadBooks();
    } catch (_) {
      books = [];
    }
    ...
    if (
      !page.classList.contains('collection-page') ||
      !document.getElementById('collectionTrophyList')
    ) {
      page.dataset.collectionTrophyDetailsRendering = '';
      return;
    }
    ...
    list.innerHTML = events.slice(0, 20).map(function (event) {
      var book = books.filter(function (item) {
        return item.id === event.bookId;
      })[0];

      var stat = metrics(event.bookId, event);
      var rarity = String(event.rarity || 'Common');

      var sessions = stat.sessions.length
        ...
        : '<li>No time entries retained</li>';

      return (
        '<article class="collection-trophy-detail rarity-' +
        raritySlug(rarity) +
        '">' +

        '<button type="button" class="collection-trophy-toggle" ' +
        'aria-expanded="false">' +

        '<span class="collection-trophy-toggle-title">' +
        '<b>★ ' + escapeHtml(event.title || 'Completed book') + '</b>' +
        '<em>' + escapeHtml(bossName(book, event)) + '</em>' +
        '</span>' +
        ...
        stat.totalXP + ' XP · ' +
        stat.gold + ' gold · ' +
        escapeHtml(event.loot || 'Trophy') +
        '</p>' +

        '<p><b>Rarity:</b> ' +
        escapeHtml(rarity) +
        ' · <b>Defeated:</b> ' +
        escapeHtml(formatDate(trophyDate(event, book, stat.sessions))) +
        ' · Crit chance used: ' +
    ...
    list.querySelectorAll('.collection-trophy-toggle').forEach(function (button) {
      button.onclick = function (event) {
        ...
        var body = card.querySelector('.collection-trophy-detail-body');
  ...
  function install() {
    ...
    if (!page || page.dataset.collectionTrophyDetailsReady) {
      return;
    }

    page.dataset.collectionTrophyDetailsReady = 'true';
