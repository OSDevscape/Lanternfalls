(function () {
  ...
  function install() {
    ...
    style.textContent =
      ...
      '.dash-books{display:flex;gap:8px;overflow:auto}' +
      '.dash-books img{width:48px;height:70px;object-fit:cover;border-radius:2px}' +
    ...
    function render() {
      ...
      var laterMarkup = later.length
        ? '<div id="dashLaterBooks" class="dash-books"></div>'
      ...
      view.innerHTML =
        ...
        '<main class="dash-body">' +
          ...
          '</section>' +
          '<section class="dash-card"><h2>Collections</h2><div class="dash-muted">Browse your personal collections</div></section>' +
          '<section class="dash-card"><h2>Series</h2><div class="dash-muted">Track books in a series</div></section>' +
          '<section class="dash-grid">' +
      ...
      var laterContainer = view.querySelector('#dashLaterBooks');

      if (laterContainer) {
        later.forEach(function (book) {
          var image = document.createElement('img');
          attachBookCover(image, book);
          laterContainer.appendChild(image);
        });
      }
