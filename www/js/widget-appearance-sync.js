(function () {
  ...
  async function sync() {
    ...
    try { await fs.writeFile({ path:FILE, directory:DIRECTORY, data:JSON.stringify({ accent:color(appearance().accent) }), encoding:'utf8' }); } catch (error) { console.warn('Widget appearance sync failed.', error); }
  }
